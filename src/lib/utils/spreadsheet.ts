...
// prettier-ignore
const header = [
    ["種別","ユーザーの種別にマッチします。\n0が生徒、-1がそれ以外のユーザー（教職員など）に対応します。"],
    ["学年", "ユーザーが生徒の場合、学年を表す1, 2, 3, 4, 5, 6の数字にマッチします。"],
    ["回生", "ユーザーが生徒の場合、何回生かを表す数字にマッチします。"],
    ["組", "ユーザーが生徒の場合、組を表す数字にマッチします。"],
    ["番号", "ユーザーが生徒の場合、出席番号を表す数字にマッチします。"],
    ["姓", "ユーザーの姓（上の名前）にマッチします。\nこの項目は字形の差異による影響を受けやすいため、他の項目で代用することを検討してください。"],
    ["名", "ユーザーの名（下の名前）にマッチします。\nこの項目は字形の差異による影響を受けやすいため、他の項目で代用することを検討してください。"],
    ["ロール", "ユーザーに付与するロールのIDを入力します。先頭にハイフンマイナス（-）をつけると、すでに付与されているロールを削除できます。"],
    ["ニックネーム", "ユーザーに設定するニックネームを表す、フォーマット指定子を含んだ文字列を入力します。"],
] as const
